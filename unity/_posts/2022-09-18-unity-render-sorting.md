...
![Untitled](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/4673fb51-401c-4a83-b3aa-1bea5e30a4b0/Untitled.png)
...
### Camera Sorting
Unity는 카메라 기준에 따라 지오메트리들을 정렬합니다. Tranparent 그룹에 대해서는 지오메트리를 뒤에서 앞(back-to-front) 순으로 정렬합니다. 그리고 Opaque 그룹에서는 기본적으로 앞에서 뒤(front-to-back)로 정렬합니다. 참고로, HSR 기능을 갖는 특정 GPU들은 거리 기반 정렬이 의미가 없기 때문에, 그런 경우에 대해서는 엔진 내부에서 거리 정렬을 수행하지 않습니다. 
### Engine Internal
...
여기에선 Unity 정렬 요소들에 대한 기본 사항에 대해서만 간단히 언급한 정도입니다. 각 정렬 요소들에 대한 더 자세한 내용은 Unity 메뉴얼을 통해서 반드시 확인하시기 바랍니다.
## 마무리
지금까지 Unity에서 지오메트리의 렌더링 정렬이 어떤 순서와 규칙을 가지고 일어나는지 살펴보았습니다. 앞으로 개발 도중에 겪을 수 있는 렌더링 순서 관련 이슈에 
## 레퍼런스
